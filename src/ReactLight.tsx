...
export const ReactLight = ({ lightOffWait, lightOnDelay }: ReactLightProps) => {
     ...
     return (
          <>
               ...
               {state.gameState === 'GAME_OVER' && <p data-testid="ReactionTimeDisplay">Reaction time : {state.clickReactionTime}</p>}
               {state.gameState === 'FALSE_START' && <p data-testid="FalseStartDisplay">FALSE START !</p>}
          </>
     );
};
